Project Log Assignments  
Module Two Team Project Plan:

Choose a scenario and select the four additional elements you will include.
  
  -Scenario 4: Third-Person
    
    -Player Power-up Pickups (health, power ups, etc.) (at least 3 unique types, can be placed around
     the level)
    -Enemies (Moving) (2 types, can be moving on platforms or jumping around)        
    -Obstacles (Moving): (3 unique types; scattered about the level. Will harm player or cause an
     effect)
    -Obstacles (Traps): (2 unique types; scattered about the level. Will cause a negative effect on
     player

-Brainstorm on content based on the preferred scenario in the design document.
    
    -Platformer with pickups, obstacles that are traps and move, and moving enemies

-Create a schedule or timeline for developing your game based on due dates outlined within the course.
    
    -Module 3 (Due Sunday March 15): Team Development: QA and Testing Plan.
    -Module 4 (Due Sunday March 22): Team Reflection and Alpha Release
    -Module 5 (Due Sunday March 29): Team Reflection and Beta Release
    -Module 6 (Due Sunday April 5): Team Reflection and Final Release
    -Module 7 (Due Sunday April 12): Final Project

-Determine the development goals for the Alpha and Beta Stage submissions (which assets/elements should be in place and what level 
 of functionality you are going to use to track the project progress in the Alpha and Beta stages).
    
    -Alpha: Pickups and moving enemies
    -Beta: Obstacles(Traps and Moving)

-Choose preferred communication method(s).
    
    -Discord Text Chat and Voice Calls

-Choose the frequency of communication (team meeting).
    
    -Once a week

-Choose a method for assigning and reporting on specific task items.
    
    -Traceability Matrix

Brandon Flynn, Dave Droege, Kevin Callahan, Kieran King, Tom Pienkowski all joined a voicecall to collaborate and agree upon this information. We all agreed upon a Third Person game. 
We also unanimously agreed and decided on the 4 additional features we want to use in the game. We decided with the content decided that a platformer would be the best choice.
The schedule is made of the different modules and due dates, during those periods we will complete Pickups and enemies for the Alpha. We will then complete obstacles for the beta.
We have decided discord will be the best option for communication within which we will share chats and voice call at least once a week. Finally we will use a traceability matrix to assign
and report specific tasks.
